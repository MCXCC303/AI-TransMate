...
_SYS_LANG_MAP = {
	...
	"hu": "Hungarian", "ro": "Romanian", "uk": "Ukrainian", "el": "Greek",
...
def _detect_system_lang() -> str:
	"""从系统 locale 检测语言。"""
	try:
		lc, _ = locale.getlocale(locale.LC_MESSAGES)
		if not lc:
			lc = locale.getdefaultlocale()[0] or ""
	except Exception:
		lc = os.environ.get("LANG", "")
	if lc:
		lang_code = lc.split("_")[0].lower()
		return _SYS_LANG_MAP.get(lang_code, "English")
	return "English"
...
DEFAULT_CONFIG = {
	...
	"target_lang": _detect_system_lang(),
...
_I18N_DIR = _CONTENTS_DIR / "lang"
_I18N: dict[str, dict] = {}
for _f in sorted(_I18N_DIR.glob("*.json")):
	with open(_f, "r", encoding="utf-8") as _fh:
		_I18N[_f.stem] = json.load(_fh)

class I18n:
	"""i18n 工具类，根据目标语言返回翻译文本。"""

	def __init__(self, target_lang: str = "Chinese"):
		self._lang = "zh_cn" if target_lang == "Chinese" else "en_us"

	def __call__(self, key: str) -> str:
		return _I18N.get(self._lang, {}).get(key, key)

def t(key: str, target_lang: str = "Chinese") -> str:
	"""便捷函数。"""
	return I18n(target_lang)(key)
